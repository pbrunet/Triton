...
class TestAstConversion(unittest.TestCase):
    ...
    def test_bvnode(self):
        ...
        for _ in xrange(100):
            ...
            self.assertEqual(n.evaluate(),
                             self.Triton.simplify(n, True).evaluate())
            # self.assertEqual(str(n), "(_ bv{} 8)".format(cv1 % 256))
    ...
    def test_compound(self):
        # Compound is not supported by z3. We should certainly simplify every
        # subexpression and fill it again.
        for run in xrange(100):
            cv1 = random.randint(0, 255)
            self.Triton.setConcreteSymbolicVariableValue(self.sv1, cv1)
            cv2 = random.randint(0, 255)
            self.Triton.setConcreteSymbolicVariableValue(self.sv2, cv2)
            n = self.astCtxt.compound([self.v1, self.v2])
            self.assertEqual(n.evaluate(), self.Triton.simplify(n, True).evaluate())
    ...
    def test_declare_function(self):
        # It doesn't work and I don't know how to aplplyt this function
        for run in xrange(100):
            cv1 = random.randint(0, 255)
            self.Triton.setConcreteSymbolicVariableValue(self.sv1, cv1)
            n = self.astCtxt.DeclareFunctionNode("foo", self.astCtxt.bvdecl(8))
            self.assertEqual(n.evaluate(), self.Triton.simplify(n, True).evaluate())
    ...
    def test_z3_assert(self):
        # It is not a z3 node so we can't simplify it using z3.
        # We should certainly make it possible to simplify what is inside the
        # assert and then fill it back
    ...
    def new_node(self, depth, possible):
        ...
        if op == self.astCtxt.ite:
            ...
            return op(*[self.new_node(depth, self.to_bool) for _ in xrange(nargs)])
        elif nargs == 0:
            return op
        else:
